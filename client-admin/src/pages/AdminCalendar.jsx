...
export default function AdminCalendar() {
    ...
    useEffect(() => {
        if (token) {
            fetchData();
            // Poll for live updates every 5 seconds (Matching Vendor Parity)
    ...
    const fetchData = async (silent = false) => {
        ...
        try {
            const [propsRes, bookingsRes] = await Promise.all([
                axios.get(`${API_BASE_URL}/admin/properties`, { headers: { Authorization: `Bearer ${token}` } }),
                axios.get(`${API_BASE_URL}/admin/bookings`, { headers: { Authorization: `Bearer ${token}` } })
            ]);
    ...
    const handleSelectSlot = async ({ start, end }) => {
        ...
        try {
            await axios.post(`${API_BASE_URL}/admin/bookings/lock`, {
    ...
    const handleEventAction = async (action, bookingId) => {
        try {
            if (action === 'approve') {
                await axios.post(`${API_BASE_URL}/admin/bookings/${bookingId}/approve`, {}, { headers: { Authorization: `Bearer ${token}` } });
                showSuccess('Approved', 'Booking confirmed.');
            } else if (action === 'reject') {
                await axios.post(`${API_BASE_URL}/admin/bookings/${bookingId}/reject`, {}, { headers: { Authorization: `Bearer ${token}` } });
        ...
        <div className="flex h-screen bg-gray-50 overflow-hidden pt-20">
            ...
            <div className="w-80 bg-white border-r border-gray-200 flex flex-col shadow-xl z-20 flex-shrink-0">
                <div className="p-4 border-b border-gray-100 bg-gray-50">
                    <h2 className="font-extrabold text-lg text-gray-800 mb-1">Admin Calendar</h2>
                    <p className="text-xs text-gray-500">Manage all bookings & properties</p>
                </div>
